...
class DroneVisualizerApp {
    ...
    bindEvents() {
        ...
        // Listen for serial commands from AeroConsole (triggered by UI buttons/input)
        window.addEventListener('serial-tx', async (e) => {
            const cmd = e.detail;
            if (cmd && this.serialManager) {
                const sent = await this.serialManager.write(cmd);
                if (!sent) {
                    this.appendConsoleLine('Failed to send command', 'log-err');
                }
            }
        });
    }
...
appendConsoleLine(text, cls = '') {
    ...
    let dir = 'RX';
    if (cls.includes('tx')) dir = 'TX';
    else if (cls.includes('err')) dir = 'ERR';
    else if (cls.includes('sys')) dir = 'SYS';

    const line = document.createElement('div');
    line.className = 'log-line';
    const ts = showTs ? `<span class="log-ts">[${new Date().toLocaleTimeString()}]</span>` : '';
    const dirClass = dir === 'TX' ? 'log-tx' : (dir === 'ERR' ? 'log-err' : (dir === 'SYS' ? 'log-sys' : 'log-rx'));
    line.innerHTML = `${ts}<span class="${dirClass}">${dir}</span><span class="log-txt">${text}</span>`;
