...
static void report_dma_mode_once(void)
{
    ...
    if (!icm42688p_dma_active()) {
        printf("[test_gyro] ICM42688P 使用轮询模式（未启用或已回退）\r\n");
        reported = true;
        return;
    }
    ...
    printf("[test_gyro] ICM42688P DMA 激活：hold=%lu，DRDY 周期≈%.2fus\r\n",
...
void test_gyro_run(void)
{
    ...
    while (1) {
        ...
        if (now - last_perf >= 1000) {
            ...
            printf("[perf] dt=%.3fs spin=%.1f dps acc=%d mag=%d strength_ok=%d cycles=%lu (max %lu) => %.2fus / %.2fus dma=%s",
                   diag->dt,
                   diag->spin_rate_dps,
                   diag->acc_valid,
                   diag->mag_used,
